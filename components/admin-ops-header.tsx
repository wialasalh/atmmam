    ...
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={() => setOpen(!open)}
        ...
        <div style={{ width:30, height:30, borderRadius:"50%", background:"#e8d9c4", display:"grid", placeItems:"center", fontSize:".8rem", fontWeight:800, color:"#5a3e2b", flexShrink:0, overflow:"hidden" }}>
          {showAvatar ? <img src={avatarUrl} alt={name} style={{ width:"100%", height:"100%", objectFit:"cover" }} onError={() => setAvatarError(true)} /> : initial}
        </div>
      ...
      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 8px)", right:0, background:"#fff", border:"1px solid #e5ecf3", borderRadius:14, boxShadow:"0 8px 24px rgba(0,0,0,.12)", minWidth:220, zIndex:9999, overflow:"hidden", direction:"rtl" }}>
          {/* معلومات المستخدم */}
          <div style={{ padding:"14px 16px", borderBottom:"1px solid #f0f4f8", display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:36, height:36, borderRadius:"50%", background:"#073766", display:"grid", placeItems:"center", flexShrink:0, overflow:"hidden" }}>
              {showAvatar ? <img src={avatarUrl} alt={name} style={{ width:"100%", height:"100%", objectFit:"cover" }} onError={() => setAvatarError(true)} /> : <span style={{ fontSize:".85rem", fontWeight:800, color:"#fff" }}>{initial}</span>}
            </div>
            <div>
