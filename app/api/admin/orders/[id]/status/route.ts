...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try { await requireRole("operator"); const { id } = await context.params; const body = await request.json(); return NextResponse.json({ data: await changeAdminOrderStatus({ ...body, orderId: id }) }); }
