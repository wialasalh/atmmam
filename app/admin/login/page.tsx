...
export default function AdminLoginPage() {
  ...
  async function signIn(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setLoading(true); setMessage(""); const data = new FormData(event.currentTarget); try { const supabase = createSupabaseBrowserClient(); const { error } = await supabase.auth.signInWithPassword({ email: String(data.get("email")), password: String(data.get("password")) }); if (error) setMessage("تعذر تسجيل الدخول. تحقق من البريد وكلمة المرور."); else window.location.href = "/admin"; } catch { setMessage("لم يتم ربط قاعدة البيانات بعد. أضف مفاتيح Supabase للمتابعة."); } finally { setLoading(false); } }
