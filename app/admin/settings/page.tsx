...
export default function SettingsPage(){
  ...
  async function loadTeam(){const response=await fetch("/api/admin/team");if(!response.ok)return false;const payload=await response.json();const list=Array.isArray(payload?.members)?payload.members:Array.isArray(payload?.data)?payload.data:Array.isArray(payload)?payload:[];if(!list.length)return false;setTeam(list.map((member:any)=>({id:member.id,full_name:member.full_name||"admin",contact:member.phone||"admin@atmmam.com.sa",role:member.role||"admin",active:member.active!==false})));setDatabaseMode(true);return true}
